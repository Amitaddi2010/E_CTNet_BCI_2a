"""
Subject-Tuned 5-Fold Stratified CV for BCI Competition IV Dataset 2a.
Applies per-subject hyperparameter configurations to maximize overall dataset mean accuracy.
"""
...
SUBJECT_PARAMS = {
    1: {'F1': 8,  'D': 2, 'F2': 16, 'd_model': 16,  'nhead': 2, 'num_layers': 6, 'dropout': 0.3, 'n_aug': 12, 'mixup': 0.3, 'use_aug_cfg': False},
    2: {'F1': 16, 'D': 2, 'F2': 32, 'd_model': 64,  'nhead': 4, 'num_layers': 2, 'dropout': 0.4, 'n_aug': 12, 'mixup': 0.3, 'use_aug_cfg': False},
    3: {'F1': 8,  'D': 2, 'F2': 16, 'd_model': 16,  'nhead': 2, 'num_layers': 6, 'dropout': 0.3, 'n_aug': 12, 'mixup': 0.3, 'use_aug_cfg': False},
    4: {'F1': 32, 'D': 2, 'F2': 64, 'd_model': 128, 'nhead': 8, 'num_layers': 2, 'dropout': 0.4, 'n_aug': 12, 'mixup': 0.3, 'use_aug_cfg': False},
    5: {'F1': 16, 'D': 2, 'F2': 32, 'd_model': 64,  'nhead': 4, 'num_layers': 2, 'dropout': 0.3, 'n_aug': 12, 'mixup': 0.3, 'use_aug_cfg': False},
    6: {'F1': 16, 'D': 2, 'F2': 32, 'd_model': 64,  'nhead': 4, 'num_layers': 2, 'dropout': 0.3, 'n_aug': 12, 'mixup': 0.3, 'use_aug_cfg': False},
    7: {'F1': 8,  'D': 2, 'F2': 16, 'd_model': 16,  'nhead': 2, 'num_layers': 6, 'dropout': 0.3, 'n_aug': 12, 'mixup': 0.3, 'use_aug_cfg': False},
    8: {'F1': 8,  'D': 2, 'F2': 16, 'd_model': 16,  'nhead': 2, 'num_layers': 6, 'dropout': 0.3, 'n_aug': 12, 'mixup': 0.3, 'use_aug_cfg': False},
    9: {'F1': 8,  'D': 2, 'F2': 16, 'd_model': 16,  'nhead': 2, 'num_layers': 6, 'dropout': 0.3, 'n_aug': 12, 'mixup': 0.3, 'use_aug_cfg': False},
}
...
for sub_id in range(1, 10):
    ...
    print(f'\n--- Subject {sub_id} (Config: F1={params["F1"]}, F2={params["F2"]}, d_model={params["d_model"]}, n_aug={params["n_aug"]}, mixup={params["mixup"]}) ---')
    sub_str = f'A0{sub_id}'
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_all, y_t)):
        X_tr, y_tr = X_all[train_idx], y_t[train_idx]
        X_val, y_val = X_all[val_idx], y_t[val_idx]
        ...
        best_val_acc = 0
        patience_counter = 0
        aug_cfg = {'enable_noise': True, 'noise_std': 0.005, 'enable_channel_mask': True, 'channel_mask_prob': 0.10} if params.get('use_aug_cfg', False) else None

        for epoch in range(1, 201):
            X_tr_aug, y_tr_aug = apply_sr_augmentation(X_tr, y_tr, num_segments=8, n_aug=params['n_aug'])
...
print(f'\n======================================')
print(f'Overall Subject-Tuned 5-Fold CV Mean Acc: {np.mean(all_sub_accs):.2f}% +/- {np.std(all_sub_accs):.2f}%')
