...
from models import CTNet, ShallowFBCSPNet, EEGNet
...
def test_subject_2(F1, F2, d_model, nhead, n_aug, mixup_alpha):
    print(f'Testing: F1={F1}, F2={F2}, d_model={d_model}, n_aug={n_aug}, mixup={mixup_alpha}')
    sub_id = 2
    t_file = os.path.join(data_dir, f'A0{sub_id}T.gdf')
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_all, y_t)):
        ...
        X_tr_aug, y_tr_aug = apply_sr_augmentation(X_tr, y_tr, num_segments=8, n_aug=n_aug)
        
        tr_ds = EEGDataset(X_tr_aug, y_tr_aug, is_train=True)
        val_ds = EEGDataset(X_val, y_val, is_train=False)
        ...
        model = CTNet(in_channels=22, time_steps=1000, n_classes=4, F1=F1, D=2, F2=F2,
                      d_model=d_model, nhead=nhead, num_layers=2, dropout=0.5).to(device)
        ...
        for epoch in range(1, 121):
            ...
            for x, _, y_b in tr_loader:
                ...
                mx, ya, yb, lam = mixup_data(x, y_b, alpha=mixup_alpha)
                optimizer.zero_grad()
                out = model(mx)
        ...
        if fold >= 1: # just run 2 folds to save time
            break
            
    print(f'Mean (first 2 folds): {np.mean(fold_accs):.4f}')
    return np.mean(fold_accs)

# Original baseline parameters
test_subject_2(F1=16, F2=32, d_model=64, nhead=4, n_aug=4, mixup_alpha=0.3)
# Larger model
test_subject_2(F1=32, F2=64, d_model=128, nhead=8, n_aug=4, mixup_alpha=0.3)
# Larger augmentation
test_subject_2(F1=16, F2=32, d_model=64, nhead=4, n_aug=8, mixup_alpha=0.3)
# Both larger
test_subject_2(F1=32, F2=64, d_model=128, nhead=8, n_aug=8, mixup_alpha=0.3)
